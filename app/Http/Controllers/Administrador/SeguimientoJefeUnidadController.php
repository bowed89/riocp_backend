...
class SeguimientoJefeUnidadController extends Controller
{
    ...
    public function asignarTecnicoRevisor(Request $request)
    {
        $user = Auth::user();
        if ($user) {
            $rules = [
                ...
                'observacion_revisor' => 'required|string',
                'nro_hoja_ruta' => 'required|string',
                'solicitud_id' => 'required|integer',
                'usuario_destino_id' => 'required|integer',
                'usuario_destino_id_revisor' => 'required|integer',
            ];
            ...
            // Verificar seguimiento creado por entidad solicitante origen y actualizar
            $seguimientoOrigen = Seguimientos::where('id', $request->id_seguimiento)->first();

            if (!$solicitud) {
                return response()->json([
                    'status' => false,
                    'message' => 'No existe un seguimiento inicial.'
                ], 400);
            }
            ...
            // agregar seguimiento para la proxima unidad:: revisor
            $seguimiento2 = new Seguimientos();
            $seguimiento2->solicitud_id = $request->solicitud_id;
            $seguimiento2->usuario_origen_id = $user->id;
            $seguimiento2->usuario_destino_id = $request->usuario_destino_id_revisor;
            $seguimiento2->estado_derivado_id = 1;
            $seguimiento2->save();
