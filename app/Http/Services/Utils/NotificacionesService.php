...
{
    public function NotificacionJefeUnidad()
    {
        $user = Auth::user();
        ...
        $newNotificaciones = $resultados->count(); 

        if ($newNotificaciones > 0) {

            event(new Notificaciones($newNotificaciones));
            return [
                'status' => true,
                'message' => 'Nuevas solicitudes',
                'data' => $resultados,
                'count' => $newNotificaciones,
                'code' => 200,
            ];
        }
        event(new Notificaciones($newNotificaciones));
